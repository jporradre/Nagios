...
Descripcion: Plugin de Nagios para monitorear el tamano de tablespaces Oracle.
'''
...
def ayuda():
    msg="check_tamano_tablespaces : Plugin de Nagios para monitorear el tamano de tablespaces Oracle.\n. NOTA: El host a monitorear debe tener sqlplus y cargada la clave SSH del server Nagios." \
        "Uso: check_tamano_tablespaces   pHost pSID pBD pServerUser pBDUser pBDPass pTblss ";
    print(msg);
...
def procesar(pHost,pSID,pBD,pServerUser,pBDUser,pBDPass,pTblss):
    ...
    for tbl in colTblss:
        ...
        cmd = '. ~/.bash_profile; sqlplus -s '+pBDUser+'/'+pBDPass+' as sysdba <<< ''\
                ''"SELECT      ROUND(a.BYTES/1024000,0) - ROUND(b.BYTES/1024000)  MB''\
